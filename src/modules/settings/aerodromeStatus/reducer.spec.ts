          ...
          it('should set loading flag', () => {
            expect(
              reducer({
                saving: false,
                loading: false
              } as any, actions.aerodromeStatusLoading())
            ).toEqual({
              saving: false,
              loading: true
            });
          ...
          it('should set loaded data', () => {
            ...
            expect(
              reducer({
                  ...
                  loading: true
                } as any, actions.aerodromeStatusLoaded(data, latest)
              )
            ).toEqual({
              data,
              latest,
          ...
          it('should update the status data', () => {
            expect(
              reducer({
                ...
                } as any, actions.updateAerodromeStatus('restricted', 'Eine Landung pro Pilot pro Tag.')
              )
            ).toEqual({
              data: {
                status: 'restricted',
          ...
          it('should set the saving flag', () => {
            expect(
              reducer({
                  ...
                  saving: false
                } as any, actions.setAerodromeStatusSaving()
              )
            ).toEqual({
              data: {
                status: 'open',
          ...
          it('should reset the saving flag', () => {
            expect(
              reducer({
                  ...
                  saving: true
                } as any, actions.saveAerodromeStatusSuccess()
              )
            ).toEqual({
              data: {
                status: 'open',
        ...
        describe('SELECT_STATUS', () => {
          it('should set selected status', () => {
            expect(
              reducer({
                selected: 'status1',
              } as any, actions.selectAerodromeStatus('status2'))
            ).toEqual({
              selected: 'status2'
            });
          });
          ...
          it('should unset selected status', () => {
            expect(
              reducer({
                selected: 'status1'
              } as any, actions.selectAerodromeStatus(null as any))
            ).toEqual({
              selected: null
            });
          });
        ...
        describe('SET_CURRENT_AERODROME_STATUS', () => {
          it('should update the current status', () => {
            expect(
              ...
              } as any, actions.setCurrentAerodromeStatus({
                status: 'open',
                details: 'Willkommen!',
                by: 'Kurt Keller',
