  ...
  describe('dates', () => {
    describe('localToIsoUtc', () => {
      ...
      it('throws an error if no date given', () => {
        expect(() => dates.localToIsoUtc(null as any, '07:10')).toThrow('Date "null" does not match pattern YYYY-MM-DD');
      });
      ...
      it('throws an error if no time given', () => {
        expect(() => dates.localToIsoUtc('2015-12-24', null as any)).toThrow('Time "null" does not match pattern HH:mm');
      });
    ...
    describe('formatDateTime', () => {
      it('formats the given timestamp as date time', () => {
        ...
        expect(dates.formatDateTime(timestamp as any)).toBe('22.03.2020 14:48')
      })

      it('formats the given timestamp as date time with given locale (en)', () => {
        const timestamp = new Date('2020-03-22T13:48:00.000Z')
        expect(dates.formatDateTime(timestamp as any, 'en')).toBe('03/22/2020 2:48 PM')
      })
    })
  });
});
